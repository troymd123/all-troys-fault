  ...
  async function loadEvents() {
    ...
    try {
      ...
      list.innerHTML = upcoming.map(e => {
        ...
        return `<div class="ticket reveal in">
          <div class="ticket-date"><span class="dow">${d.dow}</span><span class="dom">${d.dom}</span><span class="mon">${d.mon}</span></div>
          <div class="ticket-info">
            <div class="venue">${esc(e.venue||"TBA")}</div>
            <div class="place">${esc(e.location||"")}</div>
            ${e.notes ? `<div class="note">${esc(e.notes)}</div>` : ""}
          </div>
          <div class="ticket-time">
            ${e.time ? formatTime(e.time) : ""}
            ${e.ticketUrl ? `<br><a class="ticket-link" href="${esc(e.ticketUrl)}" target="_blank" rel="noopener">Details &rarr;</a>` : ""}
          </div>
        </div>`;
      }).join("");
    } catch {
